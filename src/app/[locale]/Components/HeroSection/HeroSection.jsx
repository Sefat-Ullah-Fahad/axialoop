...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      init() {
        ...
        this.dist = Math.sqrt(Math.pow(this.x - centerX, 2) + Math.pow(this.y - centerY, 2));
        ...
        const greenShades = ['rgba(74, 222, 128,', 'rgba(34, 197, 94,', 'rgba(20, 83, 45,', 'rgba(134, 239, 172,'];
        this.colorBase = greenShades[Math.floor(Math.random() * greenShades.length)];
        this.opacity = Math.random() * 0.5 + 0.3;
      }

      update() {
        this.dist += this.speed * this.dir;
        if (this.dist > Math.max(canvas.width, canvas.height) / 1.5 || this.dist < 10) {
          this.dir *= -1;
        }
        ...
        const centerX = canvas.width / 2;
        const centerY = canvas.height / 2;
        let currentSize = this.minSize + (Math.sin(this.pulseVal) + 1) * (this.maxSize / 2);
        const posX = centerX + Math.cos(this.angle) * this.dist;
        const posY = centerY + Math.sin(this.angle) * this.dist;

        ctx.fillStyle = this.colorBase + this.opacity + ')';
        ctx.fillRect(posX - currentSize / 2, posY - currentSize / 2, currentSize, currentSize);
      }
    }
  ...
  return (
    ...
    <section id='hero' className="   relative w-full min-h-screen py-20 pt-32 md:pt-40 bg-black flex flex-col items-center justify-center overflow-hidden custom-bottom-border   "    >
