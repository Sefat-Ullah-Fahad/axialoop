    ...
    >
      <nav className="flex items-center bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 px-4 md:px-6 py-2 rounded-full transition-all duration-500 ease-in-out">
        ...
        <AnimatePresence mode="wait">
          {scrolled && (
            <motion.div
              ...
              exit={{ opacity: 0, width: 0, marginRight: 0, transition: { duration: 0.4, ease: "circIn" } }}
        ...
        <ul className="hidden md:flex items-center gap-7 text-[12px] font-medium text-neutral-400 uppercase tracking-[0.1em]">
          <li>
            <a onClick={() => scrollToSection('path')} className="hover:text-white cursor-pointer transition-colors duration-300">
              {t('solutions')}
            </a>
          </li>
          <li>
            <a onClick={() => scrollToSection('success')} className="hover:text-white cursor-pointer transition-colors duration-300">
              {t('caseStudies')}
            </a>
          </li>
          <li>
            <Link href="/about" className="hover:text-white cursor-pointer transition-colors duration-300">
              {t('aboutUs')}
            </Link>
          </li>
          <li>
            <Link href="/templates" className="hover:text-white cursor-pointer transition-colors duration-300">
              {t('templates')}
            </Link>
          </li>
        </ul>
        ...
        <div className="flex items-center md:pl-7  gap-3">
          ...
          >
            <motion.div className="w-5 h-[2px] bg-white" variants={line1Variants} animate={isMenuOpen ? "opened" : "closed"} />
            <motion.div className="w-5 h-[2px] bg-white" variants={line2Variants} animate={isMenuOpen ? "opened" : "closed"} />
          </button>
        </div>
      </nav>
        ...
        {isMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <div onClick={() => scrollToSection('path')} className="flex items-center justify-between py-4 border-b border-white/5 group cursor-pointer">
                <span className="text-lg text-white font-medium">{t('solutions')}</span>
                <ArrowRight size={16} className="text-gray-600 group-hover:text-white transition-all" />
              </div>
              <div onClick={() => scrollToSection('success')} className="flex items-center justify-between py-4 border-b border-white/5 group cursor-pointer">
                <span className="text-lg text-white font-medium">{t('caseStudies')}</span>
                <ArrowRight size={16} className="text-gray-600 group-hover:text-white transition-all" />
              </div>
              <Link href="/about" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between py-4 border-b border-white/5 group cursor-pointer">
                <span className="text-lg text-white font-medium">{t('aboutUs')}</span>
                <ArrowRight size={16} className="text-gray-600 group-hover:text-white transition-all" />
              </Link>
              <Link href="/templates" onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between py-4 border-b border-white/5 group cursor-pointer">
                <span className="text-lg text-white font-medium">{t('templates')}</span>
                <ArrowRight size={16} className="text-gray-600 group-hover:text-white transition-all" />
              </Link>
            </div>
          </motion.div>
